...
public final class OpusDecoder extends pdb {
    ...
    public OpusDecoder(List list) {
        super(new pcz[16], new OpusOutputBuffer[16]);
        int i;
        int i2;
        byte[] bArr = (byte[]) list.get(0);
        int length = bArr.length;
        if (length < 19) {
            throw new oxl("Header size is too small.");
        }
        int i3 = bArr[9] & 255;
        this.b = i3;
        if (i3 > 8) {
        ...
        if (bArr[18] == 0) {
            if (i3 > 2) {
                throw new oxl("Invalid Header, missing stream map.");
            }
            int i4 = i3 == 2 ? 1 : 0;
            bArr2[0] = 0;
            bArr2[1] = 1;
        ...
        if (list.size() == 3) {
            if (((byte[]) list.get(1)).length != 8 || ((byte[]) list.get(2)).length != 8) {
                throw new oxl("Invalid Codec Delay or Seek Preroll");
            }
        ...
        this.e = opusInit;
        if (opusInit == 0) {
            throw new oxl("Failed to initialize decoder");
        }
        k(5760);
    }
    ...
    @Override // defpackage.pdb
    public final /* bridge */ /* synthetic */ pda b() {
        return new OpusOutputBuffer(this);
    }

    @Override // defpackage.pdb
    public final /* bridge */ /* synthetic */ Exception c(pcz pczVar, pda pdaVar, boolean z) {
        OpusOutputBuffer opusOutputBuffer = (OpusOutputBuffer) pdaVar;
        if (z) {
            opusReset(this.e);
            this.f = pczVar.a.e == 0 ? this.c : this.d;
        }
        oug ougVar = pczVar.a;
        opusOutputBuffer.timestampUs = ougVar.e;
        ByteBuffer byteBuffer = ougVar.b;
        byteBuffer.position(byteBuffer.position() - ougVar.c);
        int opusDecode = opusDecode(this.e, ougVar.b, ougVar.c, opusOutputBuffer, 48000);
        oxl oxlVar = null;
        if (opusDecode < 0) {
            String valueOf = String.valueOf(opusGetErrorMessage(opusDecode));
            oxlVar = new oxl(valueOf.length() != 0 ? "Decode error: ".concat(valueOf) : new String("Decode error: "));
        } else {
