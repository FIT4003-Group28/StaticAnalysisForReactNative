package com.google.protos.youtube.api.innertube;
/* compiled from: PG */
/* loaded from: classes3.dex */
public final class FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint extends aopi implements aoqv {
    public static final FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint a;
    private static volatile aorb f;
    public static final aopg fetchTopicPickerEndpoint;
    public int b;
    public String c = "";
    public String d = "";
    public aopu e = aopi.emptyProtobufList();

    static {
        FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint fetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint = new FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint();
        a = fetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint;
        aopi.registerDefaultInstance(FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint.class, fetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint);
        fetchTopicPickerEndpoint = aopi.newSingularGeneratedExtension(apzg.a, fetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint, fetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint, null, 168068692, aosj.MESSAGE, FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint.class);
    }

    private FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint() {
    }

    @Override // defpackage.aopi
    protected final Object dynamicMethod(aoph aophVar, Object obj, Object obj2) {
        aoph aophVar2 = aoph.GET_MEMOIZED_IS_INITIALIZED;
        switch (aophVar.ordinal()) {
            case 0:
                return (byte) 1;
            case 1:
                return null;
            case 2:
                return newMessageInfo(a, "\u0001\u0003\u0000\u0001\u0001\u0003\u0003\u0000\u0001\u0000\u0001ဈ\u0000\u0002ဈ\u0001\u0003\u001a", new Object[]{"b", "c", "d", "e"});
            case 3:
                return new FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint();
            case 4:
                return new aopa(a);
            case 5:
                return a;
            case 6:
                aorb aorbVar = f;
                if (aorbVar == null) {
                    synchronized (FetchTopicPickerEndpointOuterClass$FetchTopicPickerEndpoint.class) {
                        aorbVar = f;
                        if (aorbVar == null) {
                            aorbVar = new aopb(a);
                            f = aorbVar;
                        }
                    }
                }
                return aorbVar;
            default:
                throw new UnsupportedOperationException();
        }
    }
}
