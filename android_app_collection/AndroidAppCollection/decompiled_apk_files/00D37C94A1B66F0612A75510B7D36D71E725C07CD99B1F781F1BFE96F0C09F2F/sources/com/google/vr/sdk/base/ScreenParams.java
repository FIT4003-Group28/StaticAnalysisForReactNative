...
public class ScreenParams {
    ...
    public ScreenParams(Display display) {
        DisplayMetrics g = axds.g(display);
        this.xMetersPerPixel = axds.f(g.xdpi);
        this.yMetersPerPixel = axds.f(g.ydpi);
        this.width = g.widthPixels;
        this.height = g.heightPixels;
        this.borderSizeMeters = axds.e(null);
        int i = this.height;
        int i2 = this.width;
        if (i > i2) {
            this.width = i;
    ...
    public static ScreenParams fromProto(Display display, Display$DisplayParams display$DisplayParams) {
        ...
        if (display$DisplayParams.hasXPpi()) {
            screenParams.xMetersPerPixel = axds.f(display$DisplayParams.getXPpi());
        }
        if (display$DisplayParams.hasYPpi()) {
            screenParams.yMetersPerPixel = axds.f(display$DisplayParams.getYPpi());
        }
        screenParams.borderSizeMeters = axds.e(display$DisplayParams);
        return screenParams;
    }
