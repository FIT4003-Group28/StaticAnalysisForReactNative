package defpackage;

import android.content.Context;
import android.text.Editable;
import android.text.TextUtils;
import android.text.style.ImageSpan;
import android.view.View;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.Comparator;
import java.util.List;
import java.util.regex.Matcher;
import java.util.regex.Pattern;
/* compiled from: PG */
/* renamed from: abkj  reason: default package */
/* loaded from: classes.dex */
public final class abkj extends akbv {
    private final ArrayList k;

    public abkj(Context context, akce akceVar, ajyi ajyiVar, ajvj ajvjVar, ajxz ajxzVar, ajvm ajvmVar) {
        super(context, ajyiVar, ajvjVar, ajxzVar, ajvmVar, new abki(akceVar));
        this.k = new ArrayList();
    }

    private final void g(String str) {
        if (!TextUtils.isEmpty(str)) {
            aopa createBuilder = assv.a.createBuilder();
            createBuilder.copyOnWrite();
            assv assvVar = (assv) createBuilder.instance;
            str.getClass();
            assvVar.b = 2;
            assvVar.c = str;
            this.k.add((assv) createBuilder.mo39build());
        }
    }

    private final void h(String str) {
        aopa createBuilder = assv.a.createBuilder();
        createBuilder.copyOnWrite();
        assv assvVar = (assv) createBuilder.instance;
        str.getClass();
        assvVar.b = 1;
        assvVar.c = str;
        this.k.add((assv) createBuilder.mo39build());
    }

    private final void i(Editable editable, int i, int i2) {
        Pattern f;
        char[] cArr = new char[Math.abs(i2 - i)];
        boolean z = false;
        editable.getChars(i, i2, cArr, 0);
        String trim = new String(cArr).trim();
        if (TextUtils.isEmpty(trim) || (f = this.c.f()) == null) {
            return;
        }
        Matcher matcher = f.matcher(trim);
        int length = trim.length();
        int i3 = 0;
        while (matcher.find()) {
            if (matcher.start() > i3) {
                aopa createBuilder = assv.a.createBuilder();
                if (!z) {
                    String j = j(trim, i3, matcher.start());
                    createBuilder.copyOnWrite();
                    assv assvVar = (assv) createBuilder.instance;
                    j.getClass();
                    assvVar.b = 1;
                    assvVar.c = j;
                } else {
                    String j2 = j(trim, length, matcher.start());
                    createBuilder.copyOnWrite();
                    assv assvVar2 = (assv) createBuilder.instance;
                    j2.getClass();
                    assvVar2.b = 1;
                    assvVar2.c = j2;
                }
                this.k.add((assv) createBuilder.mo39build());
                i3 = matcher.start();
            }
            g(this.c.d(matcher.group()));
            length = matcher.end();
            z = true;
        }
        if (!z) {
            h(trim);
        } else if (length >= trim.length()) {
        } else {
            h(j(trim, length, trim.length()));
        }
    }

    private static final String j(String str, int i, int i2) {
        return i < i2 ? str.substring(i, i2) : "";
    }

    public final assu a(final Editable editable) {
        aopa createBuilder = assu.a.createBuilder();
        View view = this.i;
        if (view != null && lj.e(view) == 1) {
            createBuilder.copyOnWrite();
            assu assuVar = (assu) createBuilder.instance;
            assuVar.b |= 1;
            assuVar.d = true;
        }
        this.k.clear();
        ImageSpan[] imageSpanArr = (ImageSpan[]) editable.getSpans(0, editable.length(), ImageSpan.class);
        if (!this.c.h() || (r2 = imageSpanArr.length) <= 0) {
            i(editable, 0, editable.length());
        } else {
            Arrays.sort(imageSpanArr, new Comparator() { // from class: abkh
                @Override // java.util.Comparator
                public final int compare(Object obj, Object obj2) {
                    Editable editable2 = editable;
                    return editable2.getSpanStart((ImageSpan) obj) - editable2.getSpanStart((ImageSpan) obj2);
                }
            });
            int i = 0;
            for (ImageSpan imageSpan : imageSpanArr) {
                int spanStart = editable.getSpanStart(imageSpan);
                if (spanStart > i) {
                    i(editable, i, spanStart);
                }
                i = editable.getSpanEnd(imageSpan);
                if (i > spanStart) {
                    int i2 = i - spanStart;
                    char[] cArr = new char[i2];
                    editable.getChars(spanStart, i, cArr, 0);
                    if (i2 > 0) {
                        g(this.c.d(new String(cArr)));
                    }
                }
            }
            if (i != editable.length()) {
                i(editable, i, editable.length());
            }
        }
        ArrayList arrayList = this.k;
        createBuilder.copyOnWrite();
        assu assuVar2 = (assu) createBuilder.instance;
        aopu aopuVar = assuVar2.c;
        if (!aopuVar.c()) {
            assuVar2.c = aopi.mutableCopy(aopuVar);
        }
        aonk.addAll((Iterable) arrayList, (List) assuVar2.c);
        return (assu) createBuilder.mo39build();
    }
}
