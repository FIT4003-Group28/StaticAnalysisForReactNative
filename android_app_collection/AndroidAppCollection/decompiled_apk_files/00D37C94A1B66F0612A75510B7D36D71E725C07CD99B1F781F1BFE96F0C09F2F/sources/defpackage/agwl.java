package defpackage;

import java.util.Collection;
import java.util.HashSet;
import java.util.Iterator;
/* compiled from: PG */
/* renamed from: agwl  reason: default package */
/* loaded from: classes.dex */
public final class agwl {
    private final agwk a;
    private final HashSet b;
    private volatile agqw c;
    private volatile int d;

    public agwl(agwk agwkVar, HashSet hashSet) {
        this.a = agwkVar;
        this.b = new HashSet(hashSet);
    }

    final synchronized int a() {
        return this.b.size();
    }

    public final agqw b() {
        if (this.c == null) {
            this.c = new agqw(this.a.a(), a(), this.d);
        }
        return this.c;
    }

    public final synchronized void c(String str) {
        zgh.m(str);
        this.b.add(str);
        this.a.d(str);
    }

    public final synchronized void d(Collection collection) {
        Iterator it = collection.iterator();
        while (it.hasNext()) {
            c((String) it.next());
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final synchronized void e() {
        this.c = null;
    }

    public final synchronized boolean f(agqz agqzVar) {
        String t = agxg.t(agqzVar.f);
        int i = 0;
        if (this.b.contains(t)) {
            if (!agqzVar.c()) {
                this.b.remove(t);
                if (this.b.isEmpty()) {
                    this.a.c().clear();
                }
            }
            int a = this.a.a();
            if (a > 0) {
                int size = a - this.b.size();
                if (size == a) {
                    this.d = 100;
                } else {
                    int i2 = (size * 100) / a;
                    if (agqzVar.c()) {
                        i2 += agqzVar.a() / a;
                    }
                    if (i2 != 0) {
                        i = i2;
                    } else if (agqzVar.d > 0) {
                        i = 1;
                    }
                    this.d = Math.min(99, i);
                }
            }
            this.c = null;
            return true;
        }
        return false;
    }

    public final synchronized void g(agqz agqzVar) {
        String t = agxg.t(agqzVar.f);
        if (this.b.remove(t)) {
            this.a.e(t);
            this.c = null;
        }
    }
}
