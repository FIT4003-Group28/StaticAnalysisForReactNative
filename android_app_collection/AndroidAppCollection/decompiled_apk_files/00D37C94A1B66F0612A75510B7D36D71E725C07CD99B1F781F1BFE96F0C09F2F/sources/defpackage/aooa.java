...
public final class aooa extends OutputStream {
    ...
    public final synchronized aoob b() {
        int i = this.f;
        byte[] bArr = this.e;
        int length = bArr.length;
        if (i >= length) {
            this.c.add(new aonz(bArr));
            this.e = a;
        } else if (i > 0) {
            byte[] bArr2 = new byte[i];
            System.arraycopy(bArr, 0, bArr2, 0, Math.min(length, i));
            this.c.add(new aonz(bArr2));
        }
        this.d += this.f;
        this.f = 0;
        return aoob.v(this.c);
    }
