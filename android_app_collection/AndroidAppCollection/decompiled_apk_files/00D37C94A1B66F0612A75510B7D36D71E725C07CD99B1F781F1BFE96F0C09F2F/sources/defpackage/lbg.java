package defpackage;

import android.content.Context;
import android.content.SharedPreferences;
import java.io.File;
import java.util.concurrent.TimeUnit;
/* compiled from: PG */
/* renamed from: lbg  reason: default package */
/* loaded from: classes3.dex */
public final class lbg implements akgt {
    private static final long a = TimeUnit.MINUTES.toSeconds(60);
    private final aadd b;
    private final SharedPreferences c;
    private final Context d;
    private boolean e;
    private ampq f;
    private ampq g;
    private final aacz h;

    static {
        TimeUnit.MINUTES.toSeconds(300L);
    }

    public lbg(Context context, SharedPreferences sharedPreferences, aacz aaczVar, aadd aaddVar) {
        this.d = context;
        sharedPreferences.getClass();
        this.c = sharedPreferences;
        aaczVar.getClass();
        this.h = aaczVar;
        aaddVar.getClass();
        this.b = aaddVar;
        this.f = amon.a;
        this.g = amon.a;
    }

    private final ampq o() {
        ampq ampqVar;
        File file;
        String[] split;
        if (!this.e && !this.f.h()) {
            try {
                String valueOf = String.valueOf(this.d.getFilesDir());
                String str = File.separator;
                StringBuilder sb = new StringBuilder(String.valueOf(valueOf).length() + 15 + String.valueOf(str).length());
                sb.append(valueOf);
                sb.append(str);
                sb.append("ondevicesuggest");
                try {
                    file = new File(sb.toString());
                } catch (NullPointerException | SecurityException e) {
                    zep.n("MainAppFileBasedOnDeviceSuggestConfig: Failed to delete the old index files.", e);
                }
                if (file.listFiles() == null) {
                    ampqVar = amon.a;
                } else {
                    String str2 = null;
                    String str3 = null;
                    for (File file2 : file.listFiles()) {
                        String absolutePath = file2.getAbsolutePath();
                        try {
                            awwc.c(absolutePath);
                            split = absolutePath.substring(absolutePath.lastIndexOf(47) + 1).split("_", -1);
                        } catch (awxu unused) {
                        }
                        if (split.length != 6 || split[2].length() != 10 || split[2].charAt(4) != '-' || split[2].charAt(7) != '-') {
                            throw new awxu();
                            break;
                        }
                        String str4 = split[2];
                        if (str3 == null || str4.compareTo(str3) > 0) {
                            str2 = absolutePath;
                            str3 = str4;
                        }
                    }
                    if (str2 != null) {
                        ampqVar = ampq.j(str2);
                    }
                    ampqVar = amon.a;
                }
                this.f = ampqVar;
                if (ampqVar.h()) {
                    this.g = ampq.j(awwc.c((String) this.f.c()));
                }
            } catch (awxu unused2) {
                this.f = amon.a;
            }
        }
        this.e = true;
        return this.f;
    }

    @Override // defpackage.akgt
    public final int a() {
        int aa = eog.aa(this.h);
        if (aa > 0) {
            return aa;
        }
        return 10;
    }

    @Override // defpackage.akgt
    public final int b() {
        int ab = eog.ab(this.h);
        if (ab > 0) {
            return ab;
        }
        return 2;
    }

    @Override // defpackage.akgt
    public final int c() {
        int ac = eog.ac(this.h);
        if (ac > 0) {
            return ac;
        }
        return 300;
    }

    @Override // defpackage.akgt
    public final long d() {
        long j;
        try {
            j = Long.parseLong(this.c.getString("on_device_suggest_download_earliest_secs", "-1"));
        } catch (NumberFormatException unused) {
            j = -1;
        }
        return j == -1 ? a : j;
    }

    @Override // defpackage.akgt
    public final ampq e() {
        return o();
    }

    @Override // defpackage.akgt
    public final ampq f() {
        aupg aupgVar = this.b.a().n;
        if (aupgVar == null) {
            aupgVar = aupg.a;
        }
        return ampq.j(aupgVar.d);
    }

    @Override // defpackage.akgt
    public final ampq g() {
        return o();
    }

    @Override // defpackage.akgt
    public final ampq h() {
        o();
        return this.g;
    }

    @Override // defpackage.akgt
    public final void i(String str) {
        this.f = ampq.j(str);
    }

    @Override // defpackage.akgt
    public final void j(String str) {
    }

    @Override // defpackage.akgt
    public final void k(String str) {
        this.g = ampq.j(str);
    }

    @Override // defpackage.akgt
    public final boolean l() {
        aupg aupgVar = this.b.a().n;
        if (aupgVar == null) {
            aupgVar = aupg.a;
        }
        return aupgVar.c;
    }

    @Override // defpackage.akgt
    public final boolean m() {
        aupg aupgVar = this.b.a().n;
        if (aupgVar == null) {
            aupgVar = aupg.a;
        }
        return aupgVar.e;
    }

    @Override // defpackage.akgt
    public final void n() {
        try {
            Long.parseLong(this.c.getString("on_device_suggest_download_latest_secs", "-1"));
        } catch (NumberFormatException unused) {
        }
    }
}
