...
public abstract class sp implements AdapterView.OnItemClickListener, sx, st {
    public Rect g;

    /* JADX INFO: Access modifiers changed from: protected */
    public static sc v(ListAdapter listAdapter) {
        if (listAdapter instanceof HeaderViewListAdapter) {
            return (sc) ((HeaderViewListAdapter) listAdapter).getWrappedAdapter();
        }
        return (sc) listAdapter;
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public static boolean w(sf sfVar) {
        int size = sfVar.size();
        for (int i = 0; i < size; i++) {
            MenuItem item = sfVar.getItem(i);
