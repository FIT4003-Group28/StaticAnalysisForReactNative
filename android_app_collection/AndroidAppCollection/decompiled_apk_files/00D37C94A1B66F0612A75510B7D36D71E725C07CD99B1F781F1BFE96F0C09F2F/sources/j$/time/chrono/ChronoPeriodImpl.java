...
final class ChronoPeriodImpl implements TemporalAmount, Serializable {
    ...
    static {
        Duration$DurationUnits$$ExternalSyntheticBackport1.m(new Object[]{ChronoUnit.YEARS, ChronoUnit.MONTHS, ChronoUnit.DAYS});
    }
    ...
    public Temporal addTo(Temporal temporal) {
        ...
        if (this.months == 0) {
            int i = this.years;
            if (i != 0) {
                temporal = temporal.mo382plus(i, ChronoUnit.YEARS);
            }
        } else {
            long monthRange = monthRange();
            if (monthRange > 0) {
                temporal = temporal.mo382plus((this.years * monthRange) + this.months, ChronoUnit.MONTHS);
            } else {
                int i2 = this.years;
                if (i2 != 0) {
                    temporal = temporal.mo382plus(i2, ChronoUnit.YEARS);
                }
                temporal = temporal.mo382plus(this.months, ChronoUnit.MONTHS);
            }
        }
        int i3 = this.days;
        return i3 != 0 ? temporal.mo382plus(i3, ChronoUnit.DAYS) : temporal;
    }
