...
abstract class SliceOps {
    ...
    public static Stream makeRef(AbstractPipeline abstractPipeline, final long j, final long j2) {
        ...
        return new ReferencePipeline.StatefulOp(abstractPipeline, StreamShape.REFERENCE, flags(j2)) { // from class: j$.util.stream.SliceOps.1
            ...
            Spliterator opEvaluateParallelLazy(PipelineHelper pipelineHelper, Spliterator spliterator) {
                ...
                return ((Node) new SliceTask(this, pipelineHelper, spliterator, Nodes.castingArray(), j, j2).invoke()).mo494spliterator();
            }
            ...
            Node opEvaluateParallel(PipelineHelper pipelineHelper, Spliterator spliterator, IntFunction intFunction) {
                long exactOutputSizeIfKnown = pipelineHelper.exactOutputSizeIfKnown(spliterator);
                if (exactOutputSizeIfKnown > 0 && spliterator.hasCharacteristics(16384)) {
                    return Nodes.collect(pipelineHelper, SliceOps.sliceSpliterator(pipelineHelper.getSourceShape(), spliterator, j, j2), SliceOps.$assertionsDisabled, intFunction);
                }
                if (!StreamOpFlag.ORDERED.isKnown(pipelineHelper.getStreamAndOpFlags())) {
                    return Nodes.collect(this, unorderedSkipLimitSpliterator(pipelineHelper.wrapSpliterator(spliterator), j, j2, exactOutputSizeIfKnown), SliceOps.$assertionsDisabled, intFunction);
                }
            ...
            public Sink opWrapSink(int i, Sink sink) {
                return new Sink.ChainedReference(sink) { // from class: j$.util.stream.SliceOps.1.1
                    ...
                    public boolean cancellationRequested() {
                        if (this.m == 0 || this.downstream.cancellationRequested()) {
                            return SliceOps.$assertionsDisabled;
                        }
                        return false;
                    }
                };
    ...
    final class SliceTask extends AbstractShortCircuitTask {
        ...
        public SliceTask mo523makeChild(Spliterator spliterator) {
        ...
        public final Node mo522getEmptyResult() {
        ...
        public final Node mo521doLeaf() {
            ...
            if (isRoot()) {
                ...
                return makeNodeBuilder.mo498build();
            }
            ...
            Node mo498build = makeNodeBuilder2.mo498build();
            this.thisNodeSize = mo498build.count();
            this.completed = SliceOps.$assertionsDisabled;
        ...
        public void cancel() {
            super.cancel();
            if (this.completed) {
                setLocalResult(mo522getEmptyResult());
            }
        }

        private Node doTruncate(Node node) {
            return node.mo501truncate(this.targetOffset, this.targetSize >= 0 ? Math.min(node.count(), this.targetOffset + this.targetSize) : this.thisNodeSize, this.generator);
        }
