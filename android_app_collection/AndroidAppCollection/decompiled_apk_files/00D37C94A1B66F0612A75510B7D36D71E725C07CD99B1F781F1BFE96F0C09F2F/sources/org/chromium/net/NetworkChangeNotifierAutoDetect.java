...
public class NetworkChangeNotifierAutoDetect extends BroadcastReceiver {
    ...
    private badg mConnectivityManagerDelegate;
    ...
    private bado mWifiManagerDelegate;
    ...
    public NetworkChangeNotifierAutoDetect(Observer observer, RegistrationPolicy registrationPolicy) {
        ...
        this.mConnectivityManagerDelegate = new badg(babj.c);
        if (Build.VERSION.SDK_INT < 23) {
            this.mWifiManagerDelegate = new bado(babj.c);
        }
        this.mNetworkCallback = new badn(this);
        this.mNetworkRequest = new NetworkRequest.Builder().addCapability(12).removeCapability(15).build();
        if (Build.VERSION.SDK_INT >= 30) {
            this.mDefaultNetworkCallback = new badf(this);
        } else {
            this.mDefaultNetworkCallback = Build.VERSION.SDK_INT >= 28 ? new badh(this) : null;
        }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public static Network[] getAllNetworksFiltered(badg badgVar, Network network) {
        NetworkCapabilities c;
        Network[] allNetworks = badgVar.a.getAllNetworks();
        if (allNetworks == null) {
            allNetworks = new Network[0];
        }
        int i = 0;
        for (Network network2 : allNetworks) {
            if (!network2.equals(network) && (c = badgVar.c(network2)) != null && c.hasCapability(12)) {
                if (c.hasTransport(4)) {
                    if (badg.g(network2)) {
    ...
    */
    ...
    public long[] getNetworksAndTypes() {
        ...
        for (Network network : allNetworksFiltered) {
            ...
            i = i2 + 1;
            jArr[i2] = this.mConnectivityManagerDelegate.a(network);
        }
        return jArr;
    }
    ...
    public void register() {
        ...
        if (this.mDefaultNetworkCallback == null) {
            this.mIgnoreNextBroadcast = babj.c.registerReceiver(this, this.mIntentFilter) != null;
        }
        this.mRegistered = true;
        badn badnVar = this.mNetworkCallback;
        if (badnVar == null) {
            return;
        }
        Network[] allNetworksFiltered = getAllNetworksFiltered(badnVar.b.mConnectivityManagerDelegate, null);
        badnVar.a = null;
        if (allNetworksFiltered.length == 1 && (c = badnVar.b.mConnectivityManagerDelegate.c(allNetworksFiltered[0])) != null && c.hasTransport(4)) {
            badnVar.a = allNetworksFiltered[0];
        }
        try {
            badg badgVar = this.mConnectivityManagerDelegate;
            NetworkRequest networkRequest = this.mNetworkRequest;
            badn badnVar2 = this.mNetworkCallback;
            Handler handler = this.mHandler;
            if (Build.VERSION.SDK_INT >= 26) {
                badgVar.a.registerNetworkCallback(networkRequest, badnVar2, handler);
            } else {
                badgVar.a.registerNetworkCallback(networkRequest, badnVar2);
            }
    ...
    void setConnectivityManagerDelegateForTests(badg badgVar) {
        this.mConnectivityManagerDelegate = badgVar;
    }

    void setWifiManagerDelegateForTests(bado badoVar) {
        this.mWifiManagerDelegate = badoVar;
    }
    ...
    public void unregister() {
        ...
        badn badnVar = this.mNetworkCallback;
        if (badnVar != null) {
            this.mConnectivityManagerDelegate.f(badnVar);
        }
        ConnectivityManager.NetworkCallback networkCallback = this.mDefaultNetworkCallback;
        if (networkCallback != null) {
            this.mConnectivityManagerDelegate.f(networkCallback);
        } else {
            babj.c.unregisterReceiver(this);
        }
    }
}
