...
public class X509Util {
    ...
    private static badx sDefaultTrustManager;
    private static boolean sLoadedSystemKeyStore;
    private static File sSystemCertificateDirectory;
    private static KeyStore sSystemKeyStore;
    private static Set sSystemTrustAnchorCache;
    private static KeyStore sTestKeyStore;
    private static badx sTestTrustManager;
    private static badw sTrustStorageListener;
    ...
    private static badx createTrustManager(KeyStore keyStore) {
        ...
        try {
            for (TrustManager trustManager : trustManagerFactory.getTrustManagers()) {
                if (trustManager instanceof X509TrustManager) {
                    try {
                        return new bady((X509TrustManager) trustManager);
                    } catch (IllegalArgumentException e) {
                        bacc.d(TAG, "Error creating trust manager (" + trustManager.getClass().getName() + "): " + e, new Object[0]);
                    }
                }
            }
            bacc.d(TAG, "Could not find suitable trust manager", new Object[0]);
            return null;
        } catch (RuntimeException e2) {
            bacc.d(TAG, "TrustManagerFactory.getTrustManagers() unexpectedly threw: %s", e2);
    ...
    private static void ensureInitializedLocked() {
        ...
        if (sTrustStorageListener == null) {
            sTrustStorageListener = new badw();
            ...
            babj.c.registerReceiver(sTrustStorageListener, intentFilter);
        }
    }
        ...
        while (true) {
            ...
            if (certificate != null) {
                if (!(certificate instanceof X509Certificate)) {
                    bacc.d(TAG, "Anchor " + str + " not an X509Certificate: " + certificate.getClass().getName(), new Object[0]);
                } else {
            ...
            try {
                ...
                for (int i = 1; i < bArr.length; i++) {
                    try {
                        ...
                        bacc.f(TAG, "intermediate " + i + " failed parsing", new Object[0]);
                    }
                }
                ...
                try {
                    ...
                    if (verifyKeyUsage(x509CertificateArr[0])) {
                        synchronized (sLock) {
                            badx badxVar = sDefaultTrustManager;
                            if (badxVar == null) {
                                return new AndroidCertVerifyResult(-1);
                            }
                            try {
                                a = badxVar.a(x509CertificateArr, str, str2);
                            } catch (CertificateException e) {
                                try {
                                    a = sTestTrustManager.a(x509CertificateArr, str, str2);
                                } catch (CertificateException unused2) {
                                    bacc.e(TAG, "Failed to validate the certificate chain, error: " + e.getMessage(), new Object[0]);
