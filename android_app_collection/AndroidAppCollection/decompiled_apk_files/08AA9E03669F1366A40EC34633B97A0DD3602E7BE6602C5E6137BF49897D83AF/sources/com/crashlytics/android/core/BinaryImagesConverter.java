...
class BinaryImagesConverter {
    ...
    private JSONArray parseProcMapsJsonFromString(String str) {
        ...
        try {
            ...
            c.h().d(CrashlyticsCore.TAG, "Unable to parse proc maps string", e);
            return jSONArray;
        }
    }
    ...
    private JSONObject jsonFromMapEntryString(String str) {
        ...
        try {
            try {
                ...
                c.h().a(CrashlyticsCore.TAG, "Could not create a binary image json string", e);
                return null;
            }
        } catch (IOException e2) {
            l h = c.h();
            h.a(CrashlyticsCore.TAG, "Could not generate ID for file " + parse.path, e2);
            return null;
        }
    }
    ...
    private File correctDataPath(File file) {
        if (Build.VERSION.SDK_INT >= 9 && file.getAbsolutePath().startsWith(DATA_DIR)) {
            try {
                ...
                c.h().e(CrashlyticsCore.TAG, "Error getting ApplicationInfo", e);
                return file;
            }
        }
        return file;
    ...
    private static byte[] generateBinaryImagesJsonString(JSONArray jSONArray) {
        ...
        try {
            ...
            c.h().d(CrashlyticsCore.TAG, "Binary images string is null", e);
            return new byte[0];
        }
    }
    ...
    private static boolean isRelevant(ProcMapEntry procMapEntry) {
        return (procMapEntry.perms.indexOf(EACTags.COMPATIBLE_TAG_ALLOCATION_AUTHORITY) == -1 || procMapEntry.path.indexOf(47) == -1) ? false : true;
    }
}
