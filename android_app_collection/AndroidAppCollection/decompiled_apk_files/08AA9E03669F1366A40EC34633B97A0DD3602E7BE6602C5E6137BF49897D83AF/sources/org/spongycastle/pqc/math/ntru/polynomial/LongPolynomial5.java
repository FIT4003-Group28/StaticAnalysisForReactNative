    ...
    public LongPolynomial5 mult(TernaryPolynomial ternaryPolynomial) {
        ...
        for (int i2 = 0; i2 != ones.length; i2++) {
            ...
            for (int i7 = 0; i7 < this.coeffs.length; i7++) {
                jArr[i5][i6] = (jArr[i5][i6] + this.coeffs[i7]) & 576319980446939135L;
                i6++;
            }
        }
        ...
        for (int i8 = 0; i8 != negOnes.length; i8++) {
            ...
            for (int i13 = 0; i13 < this.coeffs.length; i13++) {
                jArr[i11][i12] = ((jArr[i11][i12] + 576601524159907840L) - this.coeffs[i13]) & 576319980446939135L;
                i12++;
            }
        }
        ...
        for (int length2 = this.coeffs.length - 1; length2 < copyOf.length; length2++) {
            if (length2 == this.coeffs.length - 1) {
                j = this.numCoeffs == 5 ? 0L : copyOf[length2] >> i18;
                i = 0;
            } else {
                j = copyOf[length2];
