...
public class OpenBSDBCrypt {
    ...
    private static byte[] decodeSaltString(String str) {
        ...
        for (int i = 0; i < length; i += 4) {
            byte b2 = decodingTable[cArr[i]];
            byte b3 = decodingTable[cArr[i + 1]];
            byte b4 = decodingTable[cArr[i + 2]];
            byte b5 = decodingTable[cArr[i + 3]];
            byteArrayOutputStream.write((b2 << 2) | (b3 >> 4));
            byteArrayOutputStream.write((b3 << 4) | (b4 >> 2));
            byteArrayOutputStream.write((b4 << 6) | b5);
        }
