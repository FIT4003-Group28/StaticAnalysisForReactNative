...
define(function (require, exports) {
    ...
    function ppPos(nd) {
        ...
        /* Pretty-print position. */
    function extrappPos(nd) {
        // console.log(nd)
        if (nd.callee.attr.enclosingFunction)
        {
            // console.log(nd.callee.attr.enclosingFunction.id)
            // console.log("-----------------------------")
            if (nd.callee.attr.enclosingFunction.id != null)
            {
                return "[" + basename(nd.attr.enclosingFile) + "]"  + nd.callee.attr.enclosingFunction.id.name;
            }
        }
        return "[" + basename(nd.attr.enclosingFile) + "] Driver code";
        //  + "@" + nd.loc.start.line + ":" + nd.range[0] + "-" + nd.range[1];
    }

    /* Pretty-print position. */
    function otherextrappPos(nd) {
        // console.log(nd)
        if (nd.name)
            return "[" + basename(nd.attr.enclosingFile) + "]" + nd.id.name;
        ...
        else
        {
            // console.log(nd)
            // console.log("-----------------------------")
            return "[" + basename(nd.attr.enclosingFile) + "]" + "Node id was undefined!";
        }
    }
    ...
    function buildAST(files) {
        ...
        sources.forEach(function (source) {
            var prog = esprima.parse(source.program, { callee: true, loc: true, range: true });
    ...
    exports.init = init;
    exports.ppPos = ppPos;
    exports.extrappPos = extrappPos;
    exports.otherextrappPos = otherextrappPos;
