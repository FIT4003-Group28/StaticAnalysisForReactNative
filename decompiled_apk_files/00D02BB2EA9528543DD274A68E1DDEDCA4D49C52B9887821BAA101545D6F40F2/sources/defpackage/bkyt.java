package defpackage;

import android.content.Context;
/* compiled from: PG */
/* renamed from: bkyt  reason: default package */
/* loaded from: classes3.dex */
final class bkyt extends cqiw<cebg> implements cqih {
    @Override // defpackage.cqiw
    protected final /* bridge */ /* synthetic */ void RI(int i, cebg cebgVar, Context context, cqiv cqivVar) {
        cebg cebgVar2 = cebgVar;
        cqivVar.a(new hsd(), hsd.d(irh.b()));
        for (int i2 = 0; i2 < cebgVar2.a().intValue(); i2++) {
            cebf b = cebgVar2.b(i2);
            if (b != null) {
                cqivVar.a(new bkys(), b);
            }
        }
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // defpackage.cqiw
    public final cqmj<cebg> a() {
        return cqgr.fY(cqgr.bq(cqrp.d(80.0d)), cqqx.D(cqgr.cd(-1), cqgr.bp(-1), cqqx.G(), cqgr.ck(C())));
    }
}
