...
public class RC4Engine implements StreamCipher {
    ...
    public byte returnByte(byte b2) {
        ...
        byte b3 = this.engineState[this.x];
        this.engineState[this.x] = this.engineState[this.y];
        this.engineState[this.y] = b3;
        return (byte) (b2 ^ this.engineState[(this.engineState[this.x] + this.engineState[this.y]) & 255]);
    }
    ...
    private void setKey(byte[] bArr) {
        ...
        for (int i4 = 0; i4 < 256; i4++) {
            ...
            byte b2 = this.engineState[i4];
            this.engineState[i4] = this.engineState[i3];
            this.engineState[i3] = b2;
            i2 = (i2 + 1) % bArr.length;
        }
    }
}
